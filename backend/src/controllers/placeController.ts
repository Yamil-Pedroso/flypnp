...
// Update a place
export const updatePlace = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
        const userData = req.user;
...
export const getSinglePlace = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { id } = req.params;
...
export const searchPlaces = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const searchMatch = await Place.find({ address: { $regex: searchWord, $options: 'i' } });
