          ...
          >
            <header className="flex shrink-0 items-center justify-between gap-4 border-b border-slate-100 px-4 py-3 sm:px-6 sm:py-4">
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                <button type="button" onClick={closeExplorer} aria-label="Close destination explorer" className="grid size-10 place-items-center rounded-full bg-slate-100 text-slate-600 transition hover:bg-slate-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-600"><X className="size-5" /></button>
              </div>
            </header>
                ...
                <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
                  {destinationRegions.map((region) => (
                    <button key={region.id} type="button" onClick={() => chooseRegion(region)} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white text-left transition hover:-translate-y-0.5 hover:border-emerald-300 hover:shadow-lg focus-visible:outline-2 focus-visible:outline-emerald-600">
                      <div className="aspect-[4/3] overflow-hidden bg-slate-100"><img src={region.image} alt={`Map representing ${region.name}`} className="size-full object-cover transition duration-300 group-hover:scale-[1.03]" /></div>
                      <div className="p-3"><span className="block font-bold text-slate-950">{region.name}</span><span className="mt-0.5 hidden text-xs text-slate-500 sm:block">{region.eyebrow}</span></div>
                    </button>
                  ))}
                </div>

                <div className="mt-6 border-t border-slate-100 pt-5">
                  <div className="mb-2 flex items-center justify-between gap-3"><p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-400">Quick destinations</p>{destination && <span className="text-xs text-slate-400">Matching “{destination}”</span>}</div>
                  <div role="listbox" aria-label="Suggested destinations" className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    {destination && destinations.length === 0 && <div className="col-span-full flex items-center gap-3 rounded-2xl bg-slate-50 p-4 text-sm text-slate-500"><Search className="size-5" />No direct match. Choose a region to explore the map.</div>}
                  </div>
                </div>
              </div>
            ) : (
              <div className={`grid min-h-0 flex-1 overflow-y-auto ${isExpanded ? "grid-rows-[auto_minmax(22rem,1fr)] lg:grid-cols-[minmax(18rem,22rem)_1fr] lg:grid-rows-1 lg:overflow-hidden" : "max-h-[calc(82vh-5rem)] lg:h-[34rem] lg:grid-cols-[19rem_1fr] lg:overflow-hidden"}`}>
                <aside className="border-b border-slate-100 p-4 lg:overflow-y-auto lg:border-b-0 lg:border-r">
                  ...
                  <div className="space-y-1">
                    ...
                    {filteredCountries.length === 0 && <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">No country found in this region.</p>}
                  </div>
                  {selectedCountry && <button type="button" onClick={closeExplorer} className="mt-4 w-full rounded-full bg-emerald-600 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-emerald-700">Use {selectedCountry.name}</button>}
                </aside>
    ...
    <>
      {selectedRegion && (
        <portals.InPortal node={mapPortalNode}>
          <Suspense fallback={<div className="grid size-full min-h-[22rem] place-items-center bg-slate-100 text-sm font-semibold text-slate-500">Loading interactive map…</div>}>
