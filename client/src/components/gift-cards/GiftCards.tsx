...
const money = (cents: number) =>
  new Intl.NumberFormat("en-CH", { style: "currency", currency: "CHF" }).format(cents / 100);
...
const PurchaseForm = ({ onPurchased, defaultName, defaultEmail }: PurchaseFormProps) => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (selectedAmount < 25 || selectedAmount > 2000) {
      setError("Choose an amount between 25 and 2,000 CHF.");
      return;
    }
    setLoading(true);
    setError("");
    try {
      ...
      if (!purchase.clientSecret) throw new Error("The secure payment session could not be created.");
      const result = await stripe.confirmCardPayment(purchase.clientSecret, {
        payment_method: { card },
      });
      if (result.error) throw new Error(result.error.message ?? "Payment failed.");
      if (result.paymentIntent?.status !== "succeeded") {
        throw new Error("Stripe has not confirmed the payment yet.");
      }
      ...
      setError(getErrorMessage(cause, "The gift card could not be purchased."));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      <div>
        <label className="text-sm font-semibold text-slate-800">Gift amount</label>
      ...
      <label className="text-sm font-semibold text-slate-800">
        ...
        <span className="relative mt-2 block">
          ...
          <textarea maxLength={500} rows={3} value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Enjoy your next adventure!" className="w-full resize-none rounded-2xl border border-slate-200 py-3 pl-11 pr-4 font-normal outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-50" />
        </span>
      </label>
      ...
      <div>
        <div className="mb-2 flex items-center justify-between text-sm font-semibold text-slate-800">
          ...
          <span className="flex items-center gap-1 text-[0.65rem] uppercase tracking-wider text-slate-400"><LockKeyhole className="size-3" /> Stripe test ready</span>
        </div>
      ...
      <button disabled={!stripe || loading} className="flex w-full items-center justify-center gap-2 rounded-full bg-gradient-to-r from-rose-600 to-rose-500 px-5 py-3.5 text-sm font-bold text-white shadow-lg shadow-rose-500/20 transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-50">
        {loading ? <><LoaderCircle className="size-4 animate-spin" /> Processing…</> : <><Gift className="size-4" /> Buy {Number.isFinite(selectedAmount) ? selectedAmount : 0} CHF gift card</>}
      </button>
    </form>
  );
};
...
const GiftCards = () => {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      toast.error(getErrorMessage(cause, "Gift card details could not be loaded."));
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  const redeem = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setRedeemCode("");
      await refresh();
      toast.success(`${money(result.amount)} added to your balance.`);
    } catch (cause) {
      toast.error(getErrorMessage(cause, "The code could not be redeemed."));
    } finally {
      setRedeeming(false);
    }
  };
    ...
    return (
      <main className="grid min-h-[70vh] place-items-center bg-slate-50 px-4">
        <div className="max-w-md rounded-[2rem] bg-white p-8 text-center shadow-xl ring-1 ring-slate-200">
          ...
          <h1 className="mt-4 text-3xl font-semibold text-slate-950">Gift cards</h1>
          <p className="mt-3 text-sm leading-6 text-slate-500">Sign in to buy, redeem and use Flypnp gift cards.</p>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-[#f7f9f8] pb-20">
      <section className="relative overflow-hidden bg-slate-950 px-4 py-16 text-white sm:px-6 sm:py-20">
        ...
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="relative mx-auto max-w-6xl">
          <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.16em] text-emerald-300"><Sparkles className="size-3.5" /> Give a future memory</span>
          <div className="mt-6 grid items-end gap-8 lg:grid-cols-[1fr_auto]">
            <div><h1 className="max-w-3xl text-4xl font-semibold tracking-[-0.05em] sm:text-6xl">Flypnp gift cards</h1><p className="mt-4 max-w-xl text-sm leading-6 text-slate-300 sm:text-base">Send travel credit, redeem a code and pay for your next booking with your balance.</p></div>
            <div className="min-w-64 rounded-[1.75rem] bg-white/10 p-5 ring-1 ring-white/15 backdrop-blur">
              <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-slate-300"><WalletCards className="size-4 text-emerald-300" /> Available balance</p>
              <p className="mt-2 text-4xl font-semibold tabular-nums">{loading ? "—" : money(summary.balance)}</p>
              <p className="mt-2 text-xs text-slate-400">Applied automatically at checkout.</p>
            </div>
          </div>
        </motion.div>
      </section>
      ...
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(20rem,.65fr)]">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
          <div className="mb-7"><p className="text-xs font-bold uppercase tracking-[0.16em] text-emerald-700">Buy a gift</p><h2 className="mt-1 text-2xl font-semibold text-slate-950">Create a gift card</h2></div>
          {stripePromise ? (
          ...
          ) : <p role="alert" className="rounded-xl bg-rose-50 p-4 text-sm text-rose-700">Stripe is not configured. Add VITE_STRIPE_PUBLIC_KEY to the client environment.</p>}
          {newCode && (
            <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
              <p className="flex items-center gap-2 font-semibold text-emerald-900"><CheckCircle2 className="size-5" /> Gift card activated</p>
              <p className="mt-2 text-sm text-emerald-800">Save or share this code with the recipient:</p>
              <button type="button" onClick={() => { void navigator.clipboard.writeText(newCode); toast.success("Code copied."); }} className="mt-3 w-full rounded-xl bg-white px-4 py-3 font-mono text-sm font-bold tracking-wider text-slate-950 ring-1 ring-emerald-200">{newCode}</button>
            </div>
          )}
        </section>
        ...
        <aside className="space-y-6">
          <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            ...
            <h2 className="mt-4 text-xl font-semibold text-slate-950">Redeem a code</h2>
            <p className="mt-2 text-sm leading-6 text-slate-500">The amount will be added to your Flypnp balance.</p>
            <form onSubmit={redeem} className="mt-5 space-y-3">
              ...
              <button disabled={redeeming || !redeemCode.trim()} className="flex w-full items-center justify-center gap-2 rounded-full bg-slate-950 px-5 py-3 text-sm font-bold text-white disabled:opacity-50">{redeeming && <LoaderCircle className="size-4 animate-spin" />} Redeem gift card</button>
            </form>
          </section>

          <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <h2 className="flex items-center gap-2 text-lg font-semibold text-slate-950"><History className="size-5 text-emerald-700" /> Recent activity</h2>
            <div className="mt-4 space-y-3">
              {!loading && summary.transactions.length === 0 && <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">No balance activity yet.</p>}
              {summary.transactions.slice(0, 6).map((transaction) => (
                <div key={transaction._id} className="flex items-center gap-3 border-b border-slate-100 pb-3 last:border-0 last:pb-0">
                  ...
                  <div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-slate-800">{transaction.description}</p><p className="mt-0.5 text-[0.68rem] text-slate-400">{new Date(transaction.createdAt).toLocaleDateString()}</p></div>
                  <span className={`text-sm font-bold tabular-nums ${transaction.amount > 0 ? "text-emerald-700" : "text-slate-800"}`}>{transaction.amount > 0 ? "+" : ""}{money(transaction.amount)}</span>
                </div>
              ))}
            </div>
          </section>
