        ...
        <section className="relative isolate overflow-hidden rounded-[2rem] bg-slate-950 px-6 py-9 text-white shadow-xl shadow-slate-950/10 sm:px-10 sm:py-12 lg:px-14">
          ...
          <div className="relative flex flex-col gap-8 sm:flex-row sm:items-end sm:justify-between">
            <div className="max-w-2xl">
              <div className="flex items-center gap-2 text-rose-50"><Heart className="size-5 fill-current" /><span className="text-xs font-semibold uppercase tracking-[0.18em]">{t("eyebrow")}</span></div>
              <h1 className="mt-4 text-4xl font-semibold tracking-tight sm:text-5xl">{t("title")}</h1>
              <p className="mt-4 max-w-xl leading-7 text-rose-50/90">{t("description")}</p>
            </div>
            <div className="flex w-fit items-center gap-3 rounded-2xl border border-white/20 bg-white/15 px-5 py-4 backdrop-blur"><Images className="size-6" /><div><p className="text-2xl font-semibold leading-none">{wishlist.length}</p><p className="mt-1 text-xs text-rose-50">{t("savedPlaces", { count: wishlist.length })}</p></div></div>
          </div>
        </section>

        <section className="pt-8 sm:pt-10">
          <div className="flex items-end justify-between gap-4">
            <div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-emerald-700">{t("collection")}</p><h2 className="mt-1 text-3xl font-semibold tracking-tight text-slate-950">{t("myWishlist")}</h2></div>
            {wishlist.length > 0 && <Link to="/" className="hidden items-center gap-2 text-sm font-semibold text-slate-500 transition hover:text-slate-950 sm:flex">{t("discoverMore")} <ArrowUpRight className="size-4" /></Link>}
          </div>

          {loading ? (
            <div className="mt-7 grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" aria-label={t("loading")}>{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-80 animate-pulse rounded-[1.75rem] bg-slate-200" />)}</div>
          ) : error ? (
            <div className="mt-7 rounded-[1.75rem] border border-rose-100 bg-rose-50 px-6 py-12 text-center" role="alert"><RefreshCw className="mx-auto size-8 text-rose-500" /><h3 className="mt-4 text-xl font-semibold text-slate-950">{t("loadErrorTitle")}</h3><p className="mt-2 text-slate-600">{t("loadErrorDescription")}</p><button type="button" onClick={() => void refresh()} className="mt-5 rounded-full bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-700">{commonT("tryAgain")}</button></div>
          ) : wishlist.length === 0 ? (
            <div className="relative mt-7 overflow-hidden rounded-[2rem] border border-slate-200 bg-white px-6 py-14 text-center shadow-sm sm:px-10 sm:py-20">
              ...
              <h3 className="mt-6 text-2xl font-semibold tracking-tight text-slate-950">{t("emptyTitle")}</h3>
              <p className="mx-auto mt-3 max-w-lg leading-7 text-slate-500">{t("emptyDescription")}</p>
              <Link to="/" className="mt-7 inline-flex items-center gap-2 rounded-full bg-slate-900 px-6 py-3 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-emerald-700"><Compass className="size-4" />{t("explore")}</Link>
            </div>
          ) : (
                ...
                return (
                  <motion.article key={wish._id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: Math.min(index * 0.04, 0.2) }} className="group min-w-0">
                    <div className="relative aspect-[4/3] overflow-hidden rounded-[1.75rem] bg-slate-200">
                      ...
                      <button type="button" onClick={() => setItemToDelete(wish)} aria-label={t("removeLabel", { title: wish.title })} className="absolute right-3 top-3 flex size-10 items-center justify-center rounded-full bg-white/90 text-slate-600 shadow-sm backdrop-blur transition hover:scale-105 hover:bg-rose-500 hover:text-white sm:opacity-0 sm:group-hover:opacity-100 sm:focus-visible:opacity-100"><Trash2 className="size-4.5" /></button>
                      <span className="absolute bottom-3 left-3 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-slate-700 backdrop-blur">{itemType === "experience" ? t("savedExperience") : t("savedStay")}</span>
                    </div>
                    <div className="px-1 pt-3">
                      <h3 className="truncate text-lg font-semibold text-slate-950">{wish.title}</h3>
                      <div className="mt-2 flex items-center justify-between gap-3">
                        <p className="flex min-w-0 items-center gap-1.5 text-sm text-slate-500"><Heart className="size-3.5 shrink-0 fill-rose-400 text-rose-400" />{t("ready")}</p>
                        {reservationPath && <Link to={reservationPath} className="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-rose-500 px-3.5 py-2 text-xs font-bold text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-rose-600"><CalendarCheck className="size-4" />{t("reserve")}</Link>}
                      </div>
                    </div>
                  </motion.article>
                );
