...
import { getBookingPaymentPath, getExperiencePaymentPath } from "../../lib/payment";
import { getErrorMessage, type Booking, type ExperienceBooking } from "../../services";
...
const Trips = () => {
  ...
  const [tripToDelete, setTripToDelete] = useState<
    { kind: "stay"; booking: Booking } | { kind: "experience"; booking: ExperienceBooking } | null
  >(null);
  ...
  const upcomingCount = upcomingTrips.length + upcomingExperiences.length;
  const pastCount = pastTrips.length + pastExperiences.length;
    ...
    <main className="min-h-screen bg-[#f6f8f6] pb-16">
      <div className="mx-auto w-full max-w-7xl px-4 pt-8 sm:px-6 sm:pt-12 lg:px-8">
        ...
        <section className="pt-8 sm:pt-10">
          ...
          ) : visibleTrips.length || visibleExperiences.length ? (
        ...
        {tripToDelete && (
          <motion.div className="fixed inset-0 z-[100] grid place-items-center bg-slate-950/60 p-4 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <motion.div role="dialog" aria-modal="true" aria-label="Delete trip" initial={{ opacity: 0, y: 18, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12, scale: 0.97 }} transition={{ type: "spring", stiffness: 360, damping: 28 }} className="relative w-full max-w-md rounded-[1.75rem] bg-white p-6 shadow-2xl sm:p-7">
              ...
              <p className="mt-2 pr-5 text-sm leading-6 text-slate-600"><span className="font-semibold text-slate-900">{tripToDelete.kind === "stay" ? tripToDelete.booking.place.title : tripToDelete.booking.experience.title}</span> will disappear from your trips. {tripToDelete.booking.status === "pending" ? "The pending reservation will also be cancelled." : "Its confirmed payment record will remain protected."}</p>
