          ...
          <section className="min-w-0 space-y-7">
            <div className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
              <div className="flex items-center justify-between gap-4">
                <div><p className="text-xs font-bold uppercase tracking-[0.16em] text-emerald-700">Step 1</p><h2 className="mt-1 text-xl font-semibold text-slate-950">Your trip</h2></div>
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                <article className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                  <div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-white text-emerald-700 shadow-sm"><CalendarDays className="size-5" /></span><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-400">Dates</p><p className="mt-1 text-sm font-semibold text-slate-950">{formatDate(checkIn)}</p><p className="mt-0.5 text-xs text-slate-500">to {formatDate(checkOut)}</p></div></div>
                  <button type="button" onClick={() => navigate(-1)} className="mt-4 text-xs font-bold text-rose-600 underline decoration-rose-200 underline-offset-4">Edit dates</button>
                </article>
                <article className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
          ...
          <aside className="order-first lg:order-none">
            <div className="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-[0_28px_70px_-40px_rgba(15,23,42,0.55)] lg:sticky lg:top-32">
              ...
              <div className="p-5 sm:p-6">
                <p className="text-xs font-bold uppercase tracking-[0.16em] text-emerald-700">Your stay</p>
                ...
                <div className="mt-6 space-y-3 border-t border-slate-200 pt-5 text-sm">
                  <div className="flex justify-between gap-4 text-slate-600"><span>Stay price</span><span className="font-medium tabular-nums text-slate-900">{price.toFixed(2)} CHF</span></div>
