...
const formatDuration = (minutes: number, hour: string, minute: string) => {
  const hours = Math.floor(minutes / 60);
  const remaining = minutes % 60;
  return `${hours ? `${hours} ${hour}` : ""}${remaining ? ` ${remaining} ${minute}` : ""}`.trim();
};
...
const ReserveExperience = ({
  ...
  const reserve = async () => {
    ...
    try {
      ...
      toast.success(t("details.paymentReady"));
      onReserved?.();
      navigate(getExperiencePaymentPath(booking));
    } catch (cause) {
      setError(getErrorMessage(cause, t("details.reserveError")));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-[0_24px_65px_-38px_rgba(15,23,42,0.55)] sm:p-6">
      <div className="flex items-end justify-between gap-4">
        <p className="text-2xl font-semibold text-slate-950">CHF {experience.price} <span className="text-sm font-normal text-slate-500">{t("details.perPerson")}</span></p>
      ...
      <div className="mt-5 grid gap-3">
        ...
        <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
          <div><p className="text-xs font-black uppercase tracking-[0.12em] text-slate-500">{t("details.participants")}</p><p className="mt-1 text-sm text-slate-600">{t("details.maximum", { count: experience.maxGuests })}</p></div>
          <div className="flex items-center gap-3">
            <button type="button" aria-label={t("details.removeParticipant")} disabled={participants <= 1} onClick={() => setParticipants((count) => Math.max(1, count - 1))} className="grid size-8 place-items-center rounded-full border border-slate-300 bg-white font-bold disabled:opacity-30">−</button>
            <span className="w-5 text-center font-bold">{participants}</span>
            <button type="button" aria-label={t("details.addParticipant")} disabled={participants >= experience.maxGuests} onClick={() => setParticipants((count) => Math.min(experience.maxGuests, count + 1))} className="grid size-8 place-items-center rounded-full border border-slate-300 bg-white font-bold disabled:opacity-30">+</button>
          </div>
        </div>
      </div>
      ...
      <button type="button" disabled={submitting} onClick={() => void reserve()} className="mt-5 flex w-full items-center justify-center gap-2 rounded-full bg-rose-500 px-5 py-3.5 text-sm font-bold text-white shadow-lg shadow-rose-500/20 transition hover:-translate-y-0.5 hover:bg-rose-600 disabled:cursor-wait disabled:opacity-60">
        {submitting ? <><LoaderCircle className="size-4 animate-spin" />{t("details.reserving")}</> : t("details.reserveExperience")}
      </button>
      ...
      <div className="mt-5 space-y-2.5 border-t border-slate-100 pt-4 text-sm">
        ...
        <div className="flex justify-between font-semibold text-slate-950"><span>{t("details.totalBeforeFees")}</span><span>CHF {total}</span></div>
      </div>
      <p className="mt-4 flex items-center justify-center gap-1.5 text-xs text-slate-400"><ShieldCheck className="size-3.5" />{t("details.noCharge")}</p>
    </div>
  );
};
...
const ExperienceDetails = () => {
  ...
  useEffect(() => {
    ...
    getExperience(slug)
      ...
      .catch((cause) => { if (active) setError(getErrorMessage(cause, t("details.notFound"))); })
      .finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [experience?.slug, getExperience, slug, t]);

  const gallery = useMemo(() => experience?.images.slice(0, 3) ?? [], [experience]);

  if (loading) {
    return <main className="flex min-h-[60vh] items-center justify-center bg-[#fbfcfb]"><p className="flex items-center gap-2 text-sm font-semibold text-slate-500"><LoaderCircle className="size-5 animate-spin text-emerald-600" />{t("details.loading")}</p></main>;
  }
  if (error || !experience) {
    return <main className="flex min-h-[60vh] items-center justify-center bg-[#fbfcfb] px-4 text-center"><div className="max-w-lg rounded-[2rem] border border-rose-100 bg-rose-50 p-8"><Compass className="mx-auto size-9 text-rose-500" /><h1 className="mt-4 text-2xl font-semibold text-slate-950">{t("details.notFound")}</h1><p className="mt-2 text-slate-600">{error}</p><button onClick={() => navigate("/experiences")} className="mt-5 rounded-full bg-slate-950 px-5 py-2.5 text-sm font-bold text-white">{t("details.browse")}</button></div></main>;
  }

  return (
    <main className="min-h-screen bg-[#fbfcfb] pb-28 lg:pb-16">
      <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        <button type="button" onClick={() => navigate(-1)} className="mb-5 inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><ArrowLeft className="size-4" />{t("details.back")}</button>
        ...
        <header className="mb-5">
          <div className="flex flex-wrap items-center gap-3 text-sm text-slate-600">
            ...
            <span className="font-medium underline decoration-slate-300 underline-offset-4">{experience.reviews} {t("details.reviews")}</span>
        ...
        <section className="grid h-80 grid-cols-1 gap-1 overflow-hidden rounded-[2rem] bg-slate-200 shadow-[0_24px_70px_-38px_rgba(15,23,42,0.55)] sm:h-[32rem] lg:grid-cols-[1.25fr_0.75fr]">
          <div className="relative overflow-hidden"><img src={gallery[0]} alt={experience.title} className="size-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-slate-950/35 via-transparent to-transparent" /><span className="absolute left-5 top-5 inline-flex items-center gap-2 rounded-full bg-slate-950/80 px-3.5 py-2 text-xs font-bold text-white backdrop-blur"><Sparkles className="size-4 text-emerald-300" />{t("details.moment")}</span></div>
          <div className="hidden grid-rows-2 gap-1 lg:grid">{gallery.slice(1).map((photo, index) => <img key={photo} src={photo} alt={t("details.viewPhoto", { title: experience.title, number: index + 2 })} className="size-full min-h-0 object-cover" />)}</div>
        </section>
        ...
        <div className="grid gap-10 pt-9 lg:grid-cols-[minmax(0,1fr)_23rem] lg:gap-14">
          <section className="min-w-0">
            <div className="flex flex-wrap gap-2">
              {[
                [Clock3, formatDuration(experience.durationMinutes, t("details.hourShort"), t("details.minuteShort"))],
                [Users, t("details.upToPeople", { count: experience.maxGuests })],
                [Globe2, experience.languages.join(" · ")],
                [experience.kind === "local-path" ? Route : Compass, experience.kind === "local-path" ? t("card.localPath") : t("details.smallGroup")],
              ].map(([Icon, label]) => {
            ...
            <div className="border-b border-slate-200 py-9">
              <p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-700">{t("details.story")}</p>
            ...
            <div className="border-b border-slate-200 py-9">
              <p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-700">{t("details.path")}</p>
              <div className="mt-6 grid gap-4">
                {experience.highlights.map((highlight, index) => (
                  <div key={highlight} className="flex gap-4 rounded-2xl bg-white p-5 ring-1 ring-slate-200">
                    ...
                    <div><p className="font-semibold text-slate-950">{highlight}</p><p className="mt-1 text-sm text-slate-500">{t("details.hostedStop", { name: experience.host.name })}</p></div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="border-b border-slate-200 py-9">
              <div className="flex items-center gap-5">
                ...
                <div><p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-700">{t("details.storyteller")}</p><h2 className="mt-1 text-2xl font-semibold text-slate-950">{t("details.hostedBy", { name: experience.host.name })}</h2><p className="mt-1 text-sm text-slate-500">{t("details.yearsHosting", { count: experience.host.yearsHosting })}</p></div>
              </div>
              <p className="mt-5 max-w-2xl leading-7 text-slate-600">{experience.host.bio}</p>
            </div>

            <div className="grid gap-5 py-9 sm:grid-cols-2">
              <article className="rounded-[1.5rem] border border-slate-200 bg-white p-6"><span className="grid size-10 place-items-center rounded-xl bg-emerald-50 text-emerald-700"><Check className="size-5" /></span><h3 className="mt-4 text-lg font-semibold text-slate-950">{t("details.included")}</h3><ul className="mt-3 space-y-2 text-sm text-slate-600">{experience.included.map((item) => <li key={item} className="flex gap-2"><span className="text-emerald-600">•</span>{item}</li>)}</ul></article>
              <article className="rounded-[1.5rem] border border-slate-200 bg-white p-6"><span className="grid size-10 place-items-center rounded-xl bg-amber-50 text-amber-700"><Compass className="size-5" /></span><h3 className="mt-4 text-lg font-semibold text-slate-950">{t("details.bring")}</h3><ul className="mt-3 space-y-2 text-sm text-slate-600">{experience.bring.map((item) => <li key={item} className="flex gap-2"><span className="text-amber-600">•</span>{item}</li>)}</ul></article>
            </div>

            <div className="rounded-[1.75rem] bg-slate-950 p-6 text-white sm:p-8">
              <div className="flex gap-4"><span className="grid size-12 shrink-0 place-items-center rounded-2xl bg-emerald-400 text-slate-950"><MapPin className="size-6" /></span><div><p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-300">{t("details.meeting")}</p><h3 className="mt-2 text-xl font-semibold">{experience.meetingPoint}</h3><p className="mt-2 text-sm leading-6 text-slate-400">{t("details.meetingText")}</p></div></div>
            </div>
          </section>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 px-4 py-3 shadow-[0_-12px_35px_-25px_rgba(15,23,42,0.55)] backdrop-blur-xl lg:hidden">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4"><div><p className="text-lg font-semibold text-slate-950">CHF {experience.price} <span className="text-sm font-normal text-slate-500">{t("details.perPerson")}</span></p><p className="mt-0.5 flex items-center gap-1 text-xs font-semibold text-slate-700"><Star className="size-3.5 fill-amber-400 text-amber-400" />{experience.rating} · {formatDuration(experience.durationMinutes, t("details.hourShort"), t("details.minuteShort"))}</p></div><button type="button" onClick={() => setMobileReserveOpen(true)} className="rounded-full bg-rose-500 px-7 py-3 text-sm font-bold text-white shadow-lg shadow-rose-500/20">{t("details.reserve")}</button></div>
      </div>

      {mobileReserveOpen && (
        <div className="fixed inset-0 z-[100] flex items-end bg-slate-950/65 backdrop-blur-sm lg:hidden" role="dialog" aria-modal="true" aria-label={t("details.reserveExperience")}>
          <button type="button" aria-label={t("details.closeOverlay")} onClick={() => setMobileReserveOpen(false)} className="absolute inset-0" />
          <div className="relative max-h-[92vh] w-full overflow-y-auto rounded-t-[2rem] bg-[#fbfcfb] p-4 pb-8 sm:mx-auto sm:mb-4 sm:max-w-md sm:rounded-[2rem]">
            <div className="mb-3 flex justify-end"><button type="button" aria-label={t("details.close")} onClick={() => setMobileReserveOpen(false)} className="grid size-9 place-items-center rounded-full bg-white text-slate-700 shadow-sm ring-1 ring-slate-200"><X className="size-5" /></button></div>
